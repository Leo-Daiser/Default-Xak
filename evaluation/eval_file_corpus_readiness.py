"""Evaluate corpus readiness diagnostics for demo_data."""
...
def run_eval(input_dir: str | Path = ROOT / "demo_data") -> dict[str, Any]:
    report = profile_corpus(input_dir)
    ...
    rows.append(
        _row(
            ...
            "PASS" if int(summary.get("total_canonical_facts") or 0) > 0 else "FAIL",
    ...
    rows.append(
        _row(
            "conflicts_detected",
            "PASS" if int(summary.get("conflict_groups") or 0) > 0 else "FAIL",
    ...
    rows.append(
        _row(
            "data_gaps_detected",
            "PASS" if int(summary.get("data_gaps") or 0) > 0 else "FAIL",
