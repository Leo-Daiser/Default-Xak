...
def build_grounding_context(payload: dict[str, Any]) -> GroundingContext:
    ...
    context = GroundingContext(no_facts_mode=not bool(facts))
...
def validate_text_against_grounding(
    ...
    *,
    ...
    if enforce_no_facts_policy and context.no_facts_mode and not violations:
        status = "fallback"
...
def _add_material(context: GroundingContext, value: Any) -> None:
    normalized = _canonical_value(_alias_lookup(value, MATERIAL_ALIASES))
    if normalized:
        context.allowed_materials.add(normalized)


def _add_regime(context: GroundingContext, value: Any) -> None:
    normalized = _canonical_value(_alias_lookup(value, REGIME_ALIASES))
    if normalized:
        context.allowed_regimes.add(normalized)


def _add_property(context: GroundingContext, value: Any) -> None:
    normalized = _canonical_value(_alias_lookup(value, PROPERTY_ALIASES))
    if normalized:
