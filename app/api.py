...
def _parsed_metadata(parsed: Any, source_url: str | None = None) -> Dict[str, Any]:
...
def _project_document_to_graph(active_graph: GraphDB | None, doc_meta: Document, chunks: List[Chunk], source_uri: str | None = None) -> Dict[str, int]:
    ...
    for chunk in chunks:
        ...
        counts = extractor.entity_counts(extraction)
        for entity in extraction.entities:
            ...
            count = counts[(entity.entity_type, entity.norm_name or entity.canonical_name.lower())]
            active_graph.upsert_entity(
                entity_id=uid,
                label=entity.canonical_name,
                properties={"type": entity.entity_type, "norm_name": entity.norm_name},
            )
        ...
        for relation in extraction.relations:
            subject_type = entity_type_by_name.get(relation.subject) or entity_type_by_name.get(normalise(relation.subject))
            object_type = entity_type_by_name.get(relation.object) or entity_type_by_name.get(normalise(relation.object))
            if not subject_type or not object_type:
                continue
            subject_uid = stable_entity_uid(workspace_uid, subject_type, relation.subject)
            object_uid = stable_entity_uid(workspace_uid, object_type, relation.object)
            active_graph.upsert_relation(
                ...
                evidence_chunk_ids=[ref.chunk_id for ref in relation.evidence] or [chunk.chunk_id],
            )
...
def _question_understanding(question: str) -> Dict[str, Any]:
    ...
    known_terms = bool(any(terms.values()) or any(constraints.values()))
...
async def refresh_graph_from_active_documents():
...
async def ingest_url(url: str):
    ...
    parsed_metadata = _parsed_metadata(parsed, source_url=url)
...
def _constraint_text(chunk: Chunk, extraction: Any) -> str:
    ...
    relation_text = " ".join(f"{rel.subject} {rel.predicate} {rel.object}" for rel in extraction.relations)
    return normalise(" ".join([chunk.text, entity_text, relation_text])).replace("ё", "е")
...
def _activity_signal_chunk(chunk: Chunk) -> bool:
    ...
    if chunk.metadata.get("chunk_kind") != "table_row" and len(re.findall(r"\bexperiment\s*:", lower)) > 1:
        return False
    if not any(_is_display_material(entity.canonical_name) for entity in extractor.extract_from_chunk(chunk).entities if entity.entity_type == "Material"):
        return False
...
def _all_index_extractions(limit_chunks: int = 600) -> List[tuple[Chunk, Any]]:
    ...
    for chunk in retrieval_engine.chunks[:limit_chunks]:
        if not _activity_signal_chunk(chunk):
            continue
        selected.append((chunk, extractor.extract_from_chunk(chunk)))
    return selected
...
async def _ask_impl(
    ...
    if ask_intent in {"material_inventory", "material_activity_summary"}:
        ...
        candidate_chunks = retrieval_engine.query(retrieval_question, top_k=top_k)
    ...
    if match_info.get("match_level") in {"partial", "no_match"} and any((match_info.get("constraints") or {}).values()):
        ...
        return _decorate_ask_response({
            ...
            "retrieval": {**retrieval_engine.stats(), **kg_diagnostics, "constraint_match": match_info, "query_rewrite": rewrite},
            "llm": llm_client.status(),
            "diagnostics": kg_diagnostics,
        }, preset_id=preset.preset_id, input_source=input_source, query_params_ignored=query_params_ignored)
